...
/* App Check tillfälligt avstängt — adblocker/browser-tillägg på desktop
 * kan blockera reCAPTCHA Enterprise-scriptet, vilket i sin tur får
 * Firebase Auth att svara `auth/internal-error` även om vi formellt är
 * i monitor mode. Tills Stage 2 (native) är på plats och Firebase
 * Console flippas till enforce kör vi utan App Check. När det blir
 * dags: hämta tillbaka import + initializeAppCheck-blocket nedan
 * från git-historien (commit 603097e). */
...
/* App Check init flyttad till disabled-block ovan (se importen).
   Återinför här tillsammans med importen när Stage 2 är klar. */
